...
exports.childCheckInValidator = [
  body('checkIn').not().isEmpty().withMessage("checkIn Date/Time is Required"),
  body('markedBy').not().isEmpty().withMessage("Marked By is Required"),
  body('markedBy').isIn(['PARENT', 'TEACHER', 'ADMIN']).withMessage("Marked By must be one of PARENT, TEACHER, ADMIN"),
  function (req, res, next) {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json(ApiResponse({}, errors.array()[0].msg, false));
